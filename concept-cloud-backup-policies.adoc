...
NOTE: StorageGRID S3 오브젝트 잠금 기능은 규정 준수 모드에 해당하는 단일 데이터 잠금 모드를 제공합니다. 동일한 거버넌스 모드는 지원되지 않으므로 사용자는 보존 설정을 무시하거나, 보호된 백업을 덮어쓰거나, 백업을 삭제할 수 없습니다.
...
* AWS의 경우 Connector를 클라우드에 구축해야 합니다

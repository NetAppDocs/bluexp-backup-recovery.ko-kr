...
[role="quick-margin-para"]
StorageGRID 또는 온-프레미스에 이미 Connector가 배포되어 있는 경우 모두 설정됩니다. 그렇지 않은 경우 ONTAP 데이터를 StorageGRID에 백업하려면 BlueXP 커넥터를 만들어야 합니다. 또한 StorageGRID에 연결할 수 있도록 커넥터에 대한 네트워크 설정을 사용자 지정해야 합니다.
...
* 팬아웃 백업 아키텍처를 사용할 때는 운영 스토리지 시스템에 다음 설정을 구성해야 합니다.
* 다중 구간 백업 아키텍처를 사용하는 경우 보조 스토리지 시스템에서 다음 설정을 구성해야 합니다.
...
** * Fan Out *: 정보가 기본 스토리지에서 보조_로, _ 소스에서 객체 스토리지로 이동합니다.
+

...
요약하면 ONTAP 작업 환경에서 볼륨에 대해 생성할 수 있는 유효한 보호 흐름이나 모든 볼륨입니다.
...
fan-out_architecture는 스냅샷 복사본을 클라우드의 대상 스토리지 시스템과 백업 오브젝트 모두에 독립적으로 전송합니다.

image:diagram-321-fanout-detailed.png["복제된 볼륨 및 백업 파일을 생성하고 업데이트하는 데 볼륨의 스냅샷 복사본이 어떻게 사용되는지를 보여주는 다이어그램"]

cascade_architecture는 스냅샷 복사본을 대상 스토리지 시스템에 먼저 전송한 다음, 이 복제본을 클라우드의 백업 객체로 전송합니다.
...
NOTE: Cascading 아키텍처를 사용할 때 사용자 지정 정책을 만들려는 경우 복제된 볼륨이 생성될 보조 시스템에서 객체 백업 정책을 생성해야 합니다. 기본 정책을 사용하는 경우 모두 설정됩니다.

BleXP 백업 및 복구 UI에서 객체 저장소 정책에 대한 백업을 생성할 수 있습니다. 의 섹션을 참조하십시오 link:task-manage-backups-ontap.html#add-a-new-backup-policy["새 백업 정책 추가"] 를 참조하십시오. System Manager 또는 ONTAP CLI를 사용하여 스냅샷 및 복제 정책을 생성해야 합니다.
...
== 서비스를 통해 귀사에 적합한 클라우드 버킷을 생성하시겠습니까

클라우드 스토리지에서 백업 파일을 생성할 때 기본적으로 백업 및 복구 서비스는 백업 파일이 상주할 버킷을 생성합니다. 특정 이름을 사용하거나 특수 속성을 할당하려면 직접 버킷을 생성할 수 있습니다. 고유한 버킷을 생성하려면 활성화 마법사를 시작하기 전에 생성해야 합니다.

BlueXP 또는 클라우드 공급업체에서 컨테이너를 생성할 수 있습니다.

* https://docs.netapp.com/us-en/bluexp-s3-storage/task-add-s3-bucket.html["BlueXP에서 S3 버킷을 생성합니다"]
...
StorageGRID 시스템에서 백업을 생성할 때 자체 S3 버킷을 생성할 수 없습니다.

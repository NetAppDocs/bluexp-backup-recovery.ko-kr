...
NOTE: Microsoft SQL Server 워크로드 보호에 대한 설명서는 기술 미리보기로 제공됩니다. 이 사전 공개 오퍼링과 함께 NetApp은 일반 출시 이전에 서비스 세부 정보, 콘텐츠 및 일정을 수정할 권한을 보유합니다.
...
백업 사본은 다음과 연관됩니다.  https://docs.netapp.com/us-en/bluexp-setup-admin/concept-netapp-accounts.html["BlueXP 계정"^] BlueXP 커넥터가 있는 곳입니다.

동일한 BlueXP 계정에 여러 개의 커넥터가 있는 경우 각 커넥터는 동일한 백업 목록을 표시합니다.

...
가능합니다 link:task-managing-backups-kubernetes.html["볼륨에 대한 백업을 시작 및 중지하거나 백업 일정을 변경합니다"^]. 또한 가능합니다 link:task-restore-backups-kubernetes.html#restoring-volumes-from-a-kubernetes-backup-file["백업 파일에서 전체 볼륨을 복원합니다"^] Azure(동일한 지역)의 동일한 또는 다른 Kubernetes 클러스터에서 새 볼륨으로.

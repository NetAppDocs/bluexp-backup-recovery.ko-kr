...
[role="quick-margin-para"]
Google Cloud Platform VPC에 이미 Connector를 구축한 경우 원하는 대로 설정할 수 있습니다. 그렇지 않은 경우, BlueXP 커넥터를 생성하여 ONTAP 데이터를 Google Cloud 스토리지에 백업해야 합니다. 또한 Connector에 대한 네트워크 설정을 사용자 지정하여 Google Cloud에 연결해야 합니다.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-3.png["세 가지"] 커넥터를 위한 네트워킹을 준비합니다
[role="quick-margin-para"]
커넥터에 필요한 네트워크 연결이 있는지 확인합니다.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-4.png["네"] 라이센스 요구 사항을 확인합니다
[role="quick-margin-para"]
Google Cloud와 BlueXP의 라이센스 요구 사항을 모두 확인해야 합니다.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-5.png["다섯"] ONTAP 클러스터를 준비합니다
[role="quick-margin-para"]
...
선택적으로 기본 Google Cloud 암호화 키를 사용하는 대신 데이터 암호화에 대해 사용자 지정 관리 키를 설정할 수 있습니다.

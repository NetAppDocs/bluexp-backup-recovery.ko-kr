...
+
Private Google Access 또는 Private Service Connect를 사용하는 경우 storage.googleapis.com 을 올바른 내부(개인) IP 주소로 지정하도록 DNS 서버가 구성되었는지 확인합니다.
...
+
Google 지침에 따라 이러한 비공개 액세스 옵션을 설정합니다. www.googleapis.com 및 storage.googleapis.com 을 올바른 내부(개인) IP 주소로 가리키도록 DNS 서버가 구성되었는지 확인합니다.

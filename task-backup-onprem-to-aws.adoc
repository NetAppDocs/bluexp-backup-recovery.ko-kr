...
[NOTE]
====
AWS GovCloud 또는 AWS 중국 지역에서 백업을 생성하는 경우 다음 두 가지 권한 집합의 All_Resource_sections에서 AWS 리소스 이름 "arn"을 변경해야 합니다.

* AWS GovCloud 지역의 경우 "AWS"를 "AWS-us-gov"로 변경해야 합니다(예 `arn:aws-us-gov:s3:::netapp-backup-*`.
* AWS 중국 지역의 경우 "AWS"를 "AWS-cn"으로 변경해야 합니다(예 `arn:aws-cn:s3:::netapp-backup-*`.
...
고유한 버킷을 생성하는 경우 "NetApp-backup"의 버킷 이름을 사용해야 합니다. 사용자 정의 이름을 사용해야 하는 경우 를 편집합니다 `ontapcloud-instance-policy-netapp-backup` 기존 CVO에 대한 IAMRole로 다음 목록을 S3 권한에 추가합니다.
...
[NOTE]
====
사용자 지정 버킷이 AWS GovCloud 또는 AWS 중국 지역에 있는 경우 사용 권한의 All_Resource_section에서 AWS 리소스 이름 "arn"을 변경해야 합니다.

* AWS GovCloud 지역의 경우 "AWS"를 "AWS-us-gov"로 변경해야 합니다(예 `arn:aws-us-gov:s3:::*`.
* AWS 중국 지역의 경우 "AWS"를 "AWS-cn"으로 변경해야 합니다(예 `arn:aws-cn:s3:::-*`.
...
표준 공용 인터넷 연결을 사용하려는 경우 모든 권한은 Connector에 의해 설정되며 다른 작업은 필요하지 않습니다. 이 연결 유형은 에 나와 있습니다 link:task-backup-onprem-to-aws.html#identify-the-connection-method["첫 번째 다이어그램"].

사내 데이터 센터에서 VPC로 인터넷을 통해 보다 안전하게 연결하려면 백업 활성화 마법사에서 AWS PrivateLink 연결을 선택하는 옵션이 있습니다. VPN 또는 AWS Direct Connect를 사용하여 프라이빗 IP 주소를 사용하는 VPC 엔드포인트 인터페이스를 통해 사내 시스템을 연결하려는 경우 필요합니다. 이 연결 유형은 에 나와 있습니다 link:task-backup-onprem-to-aws.html#identify-the-connection-method["두 번째 다이어그램"].

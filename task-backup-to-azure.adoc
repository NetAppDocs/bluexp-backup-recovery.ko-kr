...
* Azure에 커넥터가 설치되어 있습니다. 커넥터는 인터넷에 접속하거나 접속하지 않고 사이트에 설치할 수 있습니다.
...
+
--
Cloud Backup BYOL 라이센스의 경우, 라이센스 기간 및 용량 동안 서비스를 사용할 수 있도록 지원하는 NetApp의 일련 번호가 필요합니다. link:task-licensing-cloud-backup.html#use-a-cloud-backup-byol-license["BYOL 라이센스 관리 방법에 대해 알아보십시오"]. 어두운 사이트에 Connector 및 Cloud Volumes ONTAP 시스템을 구축할 때는 BYOL 라이센스를 사용해야 합니다.

그리고 백업이 위치할 스토리지 공간에 대한 Microsoft Azure 구독이 있어야 합니다.

--
커넥터 요구 사항:: 커넥터는 인터넷에 접속하거나 접속하지 않고 Azure 지역에 설치할 수 있습니다. 현재 Azure 상용 사이트만 인터넷에 액세스할 수 없는 사이트에 지원됩니다.
+
--
...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-install-connector-onprem-no-internet.html["인터넷 액세스 없이 Azure 지역의 Linux 호스트에 커넥터 설치"^]

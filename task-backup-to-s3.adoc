...
커넥터 요구 사항:: 커넥터는 인터넷에 연결되어 있거나 연결되어 있지 않은 AWS 지역에 설치할 수 있습니다. 현재 AWS 상업용 사이트만 인터넷에 액세스할 수 없는 사이트에 지원됩니다.
+
--
...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-install-connector-onprem-no-internet.html["인터넷 액세스 없이 AWS 지역의 Linux 호스트에 커넥터 설치"^]

...
* 권한이 있는 BlueXP Connector를 제공하는 IAM 역할에는 최신 버전의 S3 권한이 포함됩니다 https://mysupport.netapp.com/site/info/cloud-manager-policies["BlueXP 정책"^].
...
* BlueXP에 권한을 제공하는 IAM 역할에 "S3:PutBucketPolicy" 권한을 추가합니다
...
+
백업에 다른 AWS 계정을 사용하려면 BlueXP에서 대상 AWS 계정 자격 증명을 추가하고 BlueXP에 사용 권한을 제공하는 IAM 역할에 "S3:PutBucketPolicy" 권한을 추가해야 합니다.

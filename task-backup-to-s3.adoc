...
+
--
* Cloud Manager에서 대상 AWS 계정 자격 증명을 추가합니다
* Cloud Manager에 권한을 제공하는 IAM 역할에 "S3:PutBuckPolicy" 권한을 추가합니다
...
AWS 권한이 필요합니다:: Cloud Manager에 사용 권한을 제공하는 IAM 역할에는 최신 버전의 S3 권한이 포함되어야 합니다 https://docs.netapp.com/us-en/cloud-manager-setup-admin/reference-permissions-aws.html["Cloud Manager 정책"^].
+
--
다음은 정책의 특정 사용 권한입니다.
...
+
백업에 다른 AWS 계정을 사용하려면 Cloud Manager에서 대상 AWS 계정 자격 증명을 추가하고 Cloud Manager에 권한을 제공하는 IAM 역할에 "S3:PutBuckPolicy" 권한을 추가해야 합니다.

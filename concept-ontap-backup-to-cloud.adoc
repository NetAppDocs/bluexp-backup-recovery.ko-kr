...
* 전체 볼륨 복원은 사내 ONTAP 시스템에만 지원됩니다(Cloud Volumes ONTAP 시스템은 현재 지원되지 않음).
* 파일 레벨 복구는 사내 ONTAP 및 Cloud Volumes ONTAP 시스템 모두에서 지원됩니다.
* 디렉토리/폴더 복구는 현재 지원되지 않습니다.
* 현재 볼륨을 단일 aggregate로 복원할 수 있습니다.
...
* 오브젝트 저장소의 WORM/규정 준수 모드는 현재 Amazon S3 및 StorageGRID에서만 지원됩니다. 이를 DataLock 기능이라고 하며 클라우드 공급자 인터페이스를 사용하지 않고 Cloud Backup 설정을 사용하여 관리해야 합니다.

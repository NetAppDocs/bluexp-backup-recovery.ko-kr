...
필요한 경우 백업에서 전체 _volume_, 폴더 또는 하나 이상의 _files_를 동일하거나 다른 작업 환경으로 복원할 수 있습니다.
...
현재 백업 대상이 Amazon S3일 때 지원을 사용할 수 있습니다. 다른 스토리지 제공업체 대상은 향후 릴리스에 추가될 예정입니다.
...
* 오브젝트 저장소의 WORM/규정 준수 모드는 현재 Amazon S3 및 StorageGRID에서만 지원됩니다. 이를 DataLock 기능이라고 합니다.

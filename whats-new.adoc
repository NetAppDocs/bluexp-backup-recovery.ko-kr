...
[IMPORTANT]
====
중요한 보안 개선 사항으로 인해 이제 Connector는 퍼블릭 클라우드 환경 내에서 백업 및 복구 리소스를 관리하기 위해 두 개의 추가 엔드포인트에 대한 아웃바운드 인터넷 액세스를 필요로 합니다. 방화벽의 "허용" 목록에 이러한 끝점이 추가되지 않은 경우 UI에 "서비스를 사용할 수 없음" 또는 "서비스 상태 확인 실패"에 대한 오류가 표시됩니다.

...
.인덱싱된 카탈로그 v2 기능
2025년 2월에 출시된 인덱싱 카탈로그 v2에는 더 효율적이고 사용하기 쉬운 새로운 기능이 추가되었습니다. 이 버전은 성능이 크게 향상되었으며 모든 신규 고객에게 기본적으로 사용됩니다.
...
** 하나 이상의 작업 환경이 이미 인덱싱된 경우 복원 대시보드의 _Search & Restore_ 아래에서 * 인덱싱 설정 * 을 클릭합니다.

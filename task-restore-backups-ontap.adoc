...
ONTAP 9.13.0 이상을 사용하여 클라우드 백업을 Cloud Volumes ONTAP 시스템으로 복원하는 경우 "빠른 복원" 작업을 수행할 수 있습니다. 빠른 복원은 가능한 한 빨리 볼륨에 대한 액세스를 제공해야 하는 재해 복구 상황에 이상적입니다. 빠른 복원은 전체 백업 파일을 복원하는 대신 백업 파일의 메타데이터를 볼륨으로 복원합니다. 빠른 복원은 성능이나 지연 시간에 민감한 애플리케이션에는 권장되지 않으며 아카이빙된 스토리지의 백업에는 지원되지 않습니다.
...
+
필요에 따라 사용자 지정 태그를 검색하여 특정 태그가 있는 볼륨만 나열합니다.

+
image:screenshot_restore_select_volume_snapshot2.png["복원할 작업 환경, 볼륨 및 볼륨 백업 파일을 선택하는 스크린샷."]
...
+
ONTAP 9.13.0 이상을 사용하여 오브젝트 스토리지에서 Cloud Volumes ONTAP 시스템으로 백업을 복원할 때 "빠른 복원" 작업을 수행할 수 있습니다.
...
* 빠른 복원은 클라우드 백업이 생성된 소스 시스템에서 ONTAP 9.12.1 이상을 실행 중인 경우에만 FlexGroup 볼륨에 대해 지원됩니다. SnapLock 볼륨의 빠른 복원은 클라우드 백업이 생성된 소스 시스템에서 ONTAP 9.11.0 이상을 실행 중인 경우에만 지원됩니다. FlexVol 볼륨에 대한 최소 요구사항은 없습니다.
